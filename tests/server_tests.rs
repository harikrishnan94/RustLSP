...
pub fn test_run_lsp_server() {
	...
	let server_listener = thread::spawn(|| tcp_server(listener));
	...
	let ls_client = TestsLanguageClient {
		counter: 0,
...
fn tcp_server(listener: TcpListener) {
	for stream in listener.incoming() {
		...
		let conn_handler = thread::spawn(move || handle_connection(stream));
...
impl LanguageServerHandling for TestsLanguageServer {
	...
	fn hover(&mut self, _: TextDocumentPositionParams, completable: LSCompletable<Hover>) {
		...
		thread::spawn(move || {
			client_rpc_handle(&mut endpoint).telemetry_event(Value::Null).unwrap();

			let hover_str = "hover_text".to_string();
			let hover = Hover {
				contents: vec![MarkedString::String(hover_str)],
				range: None,
			};
